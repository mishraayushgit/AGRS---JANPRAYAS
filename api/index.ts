...
app.get(["/api/health", "/health"], (_req, res) => {
  res.json({
    status: "online",
    service: "Jan Prayas AI Grievance Redressal System (AGRS)",
    version: "1.0.0",
...
// Grievance analysis endpoint (support both /api/v1/analyze-grievance and /v1/analyze-grievance)
app.post(
  ["/api/v1/analyze-grievance", "/v1/analyze-grievance"],
  upload.single("file"),
  async (req, res) => {
    try {
      ...
      const userPrompt =
        "Analyze the provided citizen grievance input. Transcribe or OCR the text verbatim, perform WhisperX-grade phoneme timestamp alignment for audio, classify the exact department, evaluate public safety priority, and provide confidence metrics.";
      ...
      if (submission_type === "text") {
        ...
        contentParts.push({
          text: `This is a citizen grievance document/photo. Perform high-accuracy OCR and categorize it: ${userPrompt}`,
        });
      }
      ...
      if (!responseText) {
        throw new Error(lastModelError?.message || "Empty response returned by AI model");
      }
      ...
      if (submission_type === "audio") {
        ...
        if (!parsed.asr_metadata.segments || parsed.asr_metadata.segments.length === 0) {
          ...
          parsed.asr_metadata.duration_seconds =
            generatedSegments.length > 0 ? generatedSegments[generatedSegments.length - 1].end : 5.0;
        }
      }

      parsed.model_used =
        model_profile === "indic-tuned"
          ? "BHASHINI Indic-Tuned v3.6 (Fine-Tuned Gemini)"
          : model_profile === "custom-tuned-id"
          ? custom_model_id
          : "Gemini 3.6 Flash Foundation";
